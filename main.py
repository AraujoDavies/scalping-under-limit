...
class Wagertool:
    ...
    def extrai_valores(self):
        ...
        results = reader.readtext(
            contraste, detail=1, allowlist="0123456789Maisdemenos.,AOVIVOaovivo "
        )
        ...
        for index, result in enumerate(results):
            ...
            # print(text)
            if text == "123456789":
                self.mercado = results[index + 1][
                    1
                ]  # diferenciar Scalping Under do Migalha
        ...
        for index, odd in enumerate(ladder.keys()):
            ...
            if ladder[odd]["back"] == 0 and ladder[odd]["lay"] > 0:
                ...
                try:
                    ...
                    logging.warning(
                        "Necessário ter no minimo 3 odds em cada resistência de back/lay"
                    )
                    return False
    ...
    def migalha(self) -> str:
        ...
        if media_dinheiro_lay > media_dinheiro_back * 0.75:
            ...
            if (
                self.ladder[odd_migalha]["back"] > 0
                and self.ladder[odd_migalha]["lay"] > 0
            ):
    ...
    def scalping_under_acima_2_20(self) -> str:
        ...
        if (
            ...
            log_msg = (
                f"2. Propondo preço a ODD de @{self.odds_back[-3]} - {self.janela}"
            )
            print(log_msg)
            logging.warning(log_msg)
    ...
    def centralizar_ladders(self):
        try:
            tickoffset = pyautogui.locateOnScreen(
                "./imgs/tickoffset.jpg", confidence=0.8
            )
...
while True:
    try:
        ...
        shutil.copy(
            "./imgs/captura_janela.png",
            f'./debug_screen/{datetime.now().strftime("ERRO_%d%m_%H%M%S.png")}',
        )
        time.sleep(5)
        logging.info("procurando nova janela para trabalhar...")
        w = Wagertool()
